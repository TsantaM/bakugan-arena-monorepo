...
function OpenGateGateCardFunctionAnimation({ plane, slotId: id, slot }: { slotId: slots_id, plane: THREE.Mesh<THREE.PlaneGeometry, THREE.MeshBasicMaterial, THREE.Object3DEventMap>, slot: portalSlotsTypeElement }) {
    const mesh = plane.getObjectByName(id)
    if (!mesh) return
        OpenGateCardAnimation({
