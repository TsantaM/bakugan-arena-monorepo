...
export default function useActiveAbilityCard({ roomId }: { roomId: string }) {
    ...
    const ActiveAbilityCard = ({ roomId, abilityId, userId, bakuganKey, slot_to_move, target_slot, target }: { roomId: string, abilityId: string, userId: string, bakuganKey: string, target_slot: slots_id | '', slot_to_move: slots_id | '', target?: string | '' }) => {

        const slotOfUser = slots?.find((s) => s.bakugans.find((b) => b.key === bakuganKey && b.userId === userId))?.id

        if (socket && slotOfUser && roomState) {
            socket.emit('use-ability-card', ({ roomId, abilityId, slot: slotOfUser, userId, bakuganKey, target_slot: target_slot, slot_to_move: slot_to_move, target: target }))
        }
    }
