...
export const PyrusAquosHaos: abilityCardsType = {
    ...
    onActivate({ roomState, userId }) {
        ...
        CombinationTripleEffect({ animationsForReplay: roomState.animationsForReplay, animations: roomState.animations, attribut_one: 'Pyrus', attribut_two: 'Aquos', attribut_tree: 'Haos', portalSlots: portalSlots, userId: userId, turn: roomState.turnState.turnCount })
        return null
    },
...
export const VentusSubterraDarkus: abilityCardsType = {
    ...
    onActivate({ roomState, userId }) {
        ...
        const portalSlots = roomState?.protalSlots
        if (!portalSlots) return failed
        CombinationTripleEffect({ animationsForReplay: roomState.animationsForReplay, animations: roomState.animations, attribut_one: 'Ventus', attribut_two: 'Subterra', attribut_tree: 'Darkus', portalSlots: portalSlots, userId: userId, turn: roomState.turnState.turnCount })
        return null
    },
    activationConditions({ roomState, userId }) {
        if (!roomState) return false
