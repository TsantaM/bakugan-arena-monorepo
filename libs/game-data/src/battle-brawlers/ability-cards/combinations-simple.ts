...
export const SubterraPyrus: abilityCardsType = {
    ...
    onActivate({ roomState, userId, bakuganKey, slot }) {
        ...
        CombinationSimpleFunction({ animationsForReplay: roomState.animationsForReplay, animations: roomState?.animations, slotOfGate: slotOfGate, attribut: 'Pyrus', attributWeak: 'Subterra', bakuganKey: bakuganKey, userId: userId, turn: roomState.turnState.turnCount })
        return null
...
export const SubterraHaos: abilityCardsType = {
    ...
    onActivate({ roomState, userId, bakuganKey, slot }) {
        ...
        if (!roomState) return failed
        if (!slotOfGate) return failed
        CombinationSimpleFunction({ animationsForReplay: roomState.animationsForReplay, animations: roomState?.animations, slotOfGate: slotOfGate, attribut: 'Subterra', attributWeak: 'Haos', bakuganKey: bakuganKey, userId: userId, turn: roomState.turnState.turnCount })
        return null
    },
    activationConditions({ roomState, userId }) {
        if (!roomState) return false
...
export const HaosDarkus: abilityCardsType = {
    ...
    onActivate({ roomState, userId, bakuganKey, slot }) {
        ...
        if (!roomState) return failed
        if (!slotOfGate) return failed
        CombinationSimpleFunction({ animationsForReplay: roomState.animationsForReplay, animations: roomState?.animations, slotOfGate: slotOfGate, attribut: 'Haos', attributWeak: 'Darkus', bakuganKey: bakuganKey, userId: userId, turn: roomState.turnState.turnCount })
        return null
    },
    activationConditions({ roomState, userId }) {
        if (!roomState) return false
...
export const DarkusAquos: abilityCardsType = {
    ...
    onActivate({ roomState, userId, bakuganKey, slot }) {
        ...
        if (!roomState) return failed
        if (!slotOfGate) return failed
        CombinationSimpleFunction({ animationsForReplay: roomState.animationsForReplay, animations: roomState?.animations, slotOfGate: slotOfGate, attribut: 'Darkus', attributWeak: 'Aquos', bakuganKey: bakuganKey, userId: userId, turn: roomState.turnState.turnCount })
        return null
    },
    activationConditions({ roomState, userId }) {
        if (!roomState) return false
...
export const AquosVentus: abilityCardsType = {
    ...
    onActivate({ roomState, userId, bakuganKey, slot }) {
        ...
        if (!roomState) return failed
        if (!slotOfGate) return failed
        CombinationSimpleFunction({ animationsForReplay: roomState.animationsForReplay, animations: roomState?.animations, slotOfGate: slotOfGate, attribut: 'Aquos', attributWeak: 'Ventus', bakuganKey: bakuganKey, userId: userId, turn: roomState.turnState.turnCount })
        return null
    },
    activationConditions({ roomState, userId }) {
        if (!roomState) return false
...
export const VentusPyrus: abilityCardsType = {
    ...
    onActivate({ roomState, userId, bakuganKey, slot }) {
        ...
        if (!roomState) return failed
        if (!slotOfGate) return failed
        CombinationSimpleFunction({ animationsForReplay: roomState.animationsForReplay, animations: roomState?.animations, slotOfGate: slotOfGate, attribut: 'Ventus', attributWeak: 'Pyrus', bakuganKey: bakuganKey, userId: userId, turn: roomState.turnState.turnCount })
        return null
    },
    activationConditions({ roomState, userId }) {
        if (!roomState) return false
